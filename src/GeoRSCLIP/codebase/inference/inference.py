...
device = "cuda"
def build_model(model_name, ckpt_path, device, zero):
    if model_name == "ViT-B-32":
        model, _, _ = open_clip.create_model_and_transforms("ViT-B/32", pretrained="openai")
        if zero !=0:
            checkpoint = torch.load(ckpt_path, map_location=device)
            msg = model.load_state_dict(key_clearner(checkpoint))
        else:
            print("*********************** Skipping")
...
def main():
    ...
    model, img_preprocess = build_model(args.model_name, args.ckpt_path, args.device, args.zero)
